...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {//main関数
	...
	vertexResourceDesc.Dimension = D3D12_RESOURCE_DIMENSION_BUFFER;
	vertexResourceDesc.Width = sizeof(VertexData) * kVertexCount;//リソースのサイズ。今回はVector4を３頂点分」
	//バッファの場合はこれらは１にする決まり
	vertexResourceDesc.Height = 1;
	vertexResourceDesc.DepthOrArraySize = 1;
	vertexResourceDesc.MipLevels = 1;
	...
	//今回は赤を書き込んでみる
	materialData->color = Vector4(1.0f, 0.0f, 0.0f, 1.0f);//*materialData = Vector4(1.0f, 0.0f, 0.0f, 1.0f);
	...
	*transformationMatrixDataSprite = MakeIdentity4x4();
	...
	//Instancing用のTranceformationMatrixリソースを作る
	...
	while (msg.message != WM_QUIT) {
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			//選択して色が変えられる
			ImGui::Begin("Window");
			ImGui::ColorEdit3("color", &materialData->color.x);//ImGui::DragFloat3("color", &materialData->x, 0.01f);
			...
			*wvpData = worldMatrix;
			...
			Matrix4x4 worldViewprtransformojectionMatrix = Multiply(worldMatrix, Multiply(viewMatrix, projectionMatrix));
